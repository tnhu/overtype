...
function logSyntaxStatus(status) {
  ...
  if (summary === lastSyntaxStatusSummary) return;

  lastSyntaxStatusSummary = summary;

  if (status.state === 'active') {
    log(`syntax highlighting · shiki ${status.version} · ${status.url}`);
    return;
  }

  if (status.state === 'loading') {
    log(`syntax highlighting · loading · ${status.version}`);
    return;
  }

  if (status.state === 'disabled') {
    log('syntax highlighting · disabled');
    return;
  }

  if (status.state === 'custom') {
    log('syntax highlighting · custom override');
    return;
  }

  if (status.state === 'error') {
    log(`syntax highlighting · error · ${status.error?.message || 'unknown'}`);
  }
}
...
function configureAdvancedFeatures() {
  component.configure({
    statsFormatter: (stats) => `${stats.words} words · ${stats.lines} lines · L${stats.line}:C${stats.column}`,
    fileUpload: {
      enabled: true,
      mimeTypes: [],
...
function getComponentAttributes() {
  ...
  if (theme && theme !== 'solar') attributes.push(`theme="${theme}"`);
  if (mode && mode !== 'normal') attributes.push(`mode="${mode}"`);
  if (height && !component.hasAttribute('auto-resize')) attributes.push(`height="${height}"`);
  if (maxHeight) attributes.push(`max-height="${maxHeight}"`);
  if (fontSize && fontSize !== '14px') attributes.push(`font-size="${fontSize}"`);
  if (fontFamily) attributes.push(`font-family="${escapeAttribute(fontFamily)}"`);
  if (padding && padding !== '16px') attributes.push(`padding="${padding}"`);
  if (component.hasAttribute('toolbar')) attributes.push('toolbar');
  if (component.hasAttribute('show-stats')) attributes.push('show-stats');
  if (component.hasAttribute('auto-resize')) attributes.push('auto-resize');
  if (component.hasAttribute('readonly')) attributes.push('readonly');
  if (component.hasAttribute('spellcheck') && component.getAttribute('spellcheck') !== 'false') attributes.push('spellcheck');
  if (component.getAttribute('syntax-highlighting') === 'false') attributes.push('syntax-highlighting="false"');
  if (component.hasAttribute('show-active-line-raw')) attributes.push('show-active-line-raw');
  if (component.getAttribute('smart-lists') === 'false') attributes.push('smart-lists="false"');
...
function applyControlState() {
  ...
  [
    ...
    if (enabled) component.setAttribute(attribute, '');
    else component.removeAttribute(attribute);
  });
...
function handleReady() {
  ...
  updateMetrics();
  updateSnippet();
  log('ready · component initialized');
}
...
component.addEventListener('change', (event) => {
  updateMetrics();
  updateSnippet();
  log(`change · ${event.detail.value.length} chars`);
});
...
component.addEventListener('spellcheck-change', (event) => {
  syncControlsFromComponent();
  updateSnippet();
  log(`spellcheck · ${event.detail.enabled ? 'on' : 'off'}`);
});

component.addEventListener('auto-resize-change', (event) => {
  syncControlsFromComponent();
  updateSnippet();
  log(`auto-resize · ${event.detail.enabled ? 'on' : 'off'}`);
});

component.addEventListener('show-stats-change', (event) => {
  syncControlsFromComponent();
  updateSnippet();
  log(`stats · ${event.detail.enabled ? 'on' : 'off'}`);
});
